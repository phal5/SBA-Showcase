최근에 에이전트들이 코드를 더 직관적으로 이해할 수 있도록 하는 데 초점을 맞춘 새로운 아키텍처 프레임워크를 초안으로 작성했습니다.

깨어 있는 대부분의 시간에 코딩 에이전트를 사용하는 만큼, 에이전트들이 - 특히 버그를 수정할 때 - 기본적으로 같은 기능을 하는 여러 객체를 만들어내는 특이한 행동을 자주 취했고, 이는 SSOT를 깨뜨리는 등 많은 문제로 이어졌습니다.

이번 연구의 기초적인 개념은 에이전트들이 단지 LLM을 하네스와 RAG에 연결한 것일 뿐이니까, 코드를 좀 더... 벡터 포인트(또는 이 경우에는 자연어)로 '압축 가능한' 형태로 유지하면 에이전트들이 코드를 찾고 읽거나, 자신들이 하려는 작업이 이미 구현되어 있는지 확인하거나, 버그를 찾아 수정하는 데도 더 편리할 것이라는 데서 옵니다.
...
## 2. 결함 총량 추정에 활용된 각 방식과 적용 수학적 근거
