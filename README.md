...
소프트웨어융합 학부생으로서 저는 깨어 있을 때 대부분 코딩 에이전트를 사용합니다. 아키텍처를 계획해서 에이전트에게 넘기고, 다른 일을 하는 동안(대부분 다른 에이전트를 실행하면서) 돌아가도록 내버려 두는데, 에이전트란 친구들이 - 특히 디버깅할 때 - 기본적으로 같은 기능을 하는 여러 객체를 만들어내는 특이한 행동을 자주 취했고, 이는 SSOT를 깨뜨려서(보통 문제를 초래합니다) 저를 계속 괴롭혔습니다.

저는 간단한 개념을 생각해냈습니다. 에이전트는 단지 LLM을 하네스와 RAG에 연결한 것일 뿐이니까, 코드를 좀 더... 벡터 포인트(또는 이 경우에는 자연어)로 '압축 가능한' 형태로 유지하면 에이전트들이 코드를 찾고 읽거나, 자신들이 하려는 작업이 이미 구현되어 있는지 확인하거나, 버그를 찾아 수정하는 데도 더 편리할 것입니다.
